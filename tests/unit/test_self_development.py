...
def evidence():
    return [{"evidence_id": "ev-1", "digest": "sha256:" + "a" * 16, "source": "test", "observed_at": 1}]
...
def base(stage="observed"):
    return dict(
        ...
        stage=stage,
        rationale="Improve validation coverage",
        evidence=evidence(),
        test_plan=["unit-test", "replay-test"],
        rollback_plan="restore previous revision",
        risk_level="low",
    )


def test_proposal_cannot_be_approved_without_explicit_actor():
    with pytest.raises(ValidationError, match="promotion_requires_explicit_approval"):
        ImprovementProposal(**base("approved"))


def test_applied_proposal_requires_revision_and_approval():
    with pytest.raises(ValidationError, match="promotion_requires_explicit_approval"):
        ImprovementProposal(**base("applied"))
