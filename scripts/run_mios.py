"""Native MIOS application launcher.
...
def _build_web() -> None:
    ...
    if not WEB_INDEX.is_file():
        raise RuntimeError("MIOS web build did not create apps/web/out/index.html")
...
def main() -> int:
    if _web_build_is_stale():
        _build_web()
    print("MIOS is available at http://127.0.0.1:8000", flush=True)
    asyncio.run(_serve())
    return 0
