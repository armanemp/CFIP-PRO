...
| Indicators/market structure | 82% | TA-Lib + CFIP-specific structure |
...
| Agent orchestration | 63% | CFIP-owned contracts + PydanticAI adapter + centralized governance |
| Self-healing | 80% | Runtime health evaluation + protected invariant API + gated execution boundary |
| Self-development | 76% | Evidence/approval/rollback state machine + governed runtime boundary |
...
| Security/governance | 83% | Trusted hosts, CORS validation, request correlation, HSTS production boundary and approval gates |
| OSS integration fabric | 79% | OSS isolated behind CFIP-owned contracts; optional adapters remain dependency-light |
| Documentation/provenance | 82% | Canonical progress, governance and OSS ledgers maintained |
...
13. Centralized agent/tool authorization rules.
14. Request-boundary security hardening and trusted-host/CORS controls.
15. Governed self-development state machine with explicit approval and rollback invariants.
