...
export function ProfessionalChartTerminalV3({ observations: initial, symbol: initialSymbol }: { observations: MarketObservation[]; symbol: string }) {
  ...
  return <div dir={rtlLocales.has(locale)?"rtl":"ltr"} className="cfip-terminal relative flex h-full min-h-0 flex-col bg-[var(--cfip-terminal-bg)] text-[var(--cfip-terminal-text)]">
    <header className="cfip-terminal-topbar relative flex h-12 shrink-0 items-center border-b border-[var(--cfip-terminal-border)] bg-[var(--cfip-terminal-surface)] px-2">
      <button onClick={()=>toggleRail(!rail)} title={rail?t(locale,"hideRail"):t(locale,"showRail")} className="mr-2 rounded border border-[var(--cfip-terminal-border-strong)] px-2 py-1.5 text-xs">☰</button>
      <button data-terminal-trigger onClick={()=>setPanel(panel==="symbol"?null:"symbol")} className="flex min-w-[180px] items-center gap-2 rounded px-2 py-1.5 text-left hover:bg-[var(--cfip-terminal-surface-active)]"><strong className="text-[15px]">{symbol}</strong><span className="text-[10px] text-[var(--cfip-terminal-text-faint)]">{meta?.name??"Forex"}</span></button>
      ...
      <span className="mx-2 h-5 w-px bg-[var(--cfip-terminal-border-subtle)]"/>
      <div className="flex gap-1">{TIMEFRAMES.map(x=><button key={x} onClick={()=>setTf(x)} className={`rounded px-2.5 py-1.5 text-xs ${tf===x?"bg-[var(--cfip-terminal-accent)] text-white":"text-[var(--cfip-terminal-text-muted)] hover:bg-[var(--cfip-terminal-surface-active)]"}`}>{x}</button>)}<button data-terminal-trigger onClick={()=>setPanel(panel==="timeframe"?null:"timeframe")} className="rounded px-2 text-[var(--cfip-terminal-text-muted)]">⋯</button></div>
      <div className="ml-auto flex items-center gap-1"><button onClick={undoDrawing} title="Ctrl/Cmd+Z" className="rounded px-2 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">↶</button><button onClick={redoDrawing} title="Ctrl/Cmd+Y" className="rounded px-2 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">↷</button>
        <button onClick={resetView} title="R" className="rounded px-2.5 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"autoFit")}</button>
        <button onClick={()=>{const c=chartRef.current;if(!c)return;const canvas=c.takeScreenshot();const link=document.createElement("a");link.download=`cfip-${symbol.replace("/","-")}-${tf}.png`;link.href=canvas.toDataURL("image/png");link.click();}} className="rounded px-2.5 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"screenshot")}</button>
        <button onClick={toggleFullscreen} className="rounded px-2.5 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"fullscreen")}</button>
        <button data-terminal-trigger onClick={()=>setPanel(panel==="replay"?null:"replay")} className="rounded px-3 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"replay")}</button>
        <button data-terminal-trigger onClick={()=>setPanel(panel==="indicators"?null:"indicators")} className="rounded px-3 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"indicators")}</button>
        <button data-terminal-trigger onClick={()=>setPanel(panel==="chartType"?null:"chartType")} className="rounded px-3 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"chartType")}</button>
        <button data-terminal-trigger onClick={()=>setPanel(panel==="language"?null:"language")} className="rounded px-3 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{locale.toUpperCase()}</button>
        <button data-terminal-trigger onClick={()=>setPanel(panel==="settings"?null:"settings")} className="rounded px-3 py-1.5 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,"settings")}</button>
      </div>
      {panel==="replay"&&<div data-terminal-panel className="absolute right-72 top-11 z-50 w-72 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-3 shadow-2xl">
        <div className="mb-2 flex items-center justify-between text-xs"><span className="font-medium text-white">{t(locale,"replay")}</span><span className="tabular-nums text-[var(--cfip-terminal-replay-muted)]">{replay.cursor + 1} / {Math.max(1,replay.end - replay.start + 1)}</span></div>
        <div className="flex gap-1">
          <button onClick={()=>setReplay(replay.status==="playing"?replayPause(replay):replayPlay(replay))} className="rounded bg-[var(--cfip-terminal-accent-strong)] px-3 py-2 text-xs text-white">{replay.status==="playing"?"Pause":"Play"}</button>
          <button onClick={()=>setReplay(replayStep(replay,1))} className="rounded border border-[var(--cfip-terminal-border-strong)] px-3 py-2 text-xs">Step</button>
          <button onClick={()=>setReplay(replayReset(replay))} className="rounded border border-[var(--cfip-terminal-border-strong)] px-3 py-2 text-xs">Reset</button>
        </div>
        <div className="mt-3 flex flex-wrap gap-1">
          {[0.5,1,2,4,8].map(speed=><button key={speed} onClick={()=>setReplay(replaySetSpeed(replay,speed))} className={`rounded px-2 py-1 text-[10px] ${replay.speed===speed?"bg-[var(--cfip-terminal-accent)] text-white":"text-[var(--cfip-terminal-text-muted)] hover:bg-[var(--cfip-terminal-surface-active)]"}`}>{speed}×</button>)}
        </div>
        <div className="mt-2 text-[10px] text-[var(--cfip-terminal-replay-muted)]">Historical replay freezes the chart cursor while the live feed continues in the background.</div>
      </div>}
      {panel==="timeframe"&&<div data-terminal-panel className="absolute right-52 top-11 z-50 grid w-60 grid-cols-3 gap-1 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-2 shadow-2xl">{TIMEFRAMES.map(x=><button key={x} onClick={()=>{setTf(x);setPanel(null)}} className="rounded px-2 py-2 text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{x}</button>)}</div>}
      {panel==="chartType"&&<div data-terminal-panel className="absolute right-40 top-11 z-50 w-44 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-2 shadow-2xl">{CHART_KINDS.map(x=><button key={x} onClick={()=>{setKind(x);setPanel(null)}} className="block w-full rounded px-3 py-2 text-left text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{t(locale,x==="candles"?"candlestick":x)}</button>)}</div>}
      {panel==="indicators"&&<div data-terminal-panel className="absolute right-28 top-11 z-50 grid w-64 grid-cols-2 gap-1 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-2 shadow-2xl">{INDICATORS.map(x=><button key={x} onClick={()=>toggle(x)} className={`rounded px-3 py-2 text-left text-xs ${selected.includes(x)?"bg-[var(--cfip-terminal-accent-strong)] text-white":"hover:bg-[var(--cfip-terminal-surface-active)]"}`}>{x}</button>)}</div>}
      {panel==="language"&&<div data-terminal-panel className="absolute right-2 top-11 z-50 grid w-64 grid-cols-2 gap-1 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-2 shadow-2xl">{(Object.keys(localeNames) as Locale[]).map(x=><button key={x} onClick={()=>{setLocale(x);setPanel(null)}} className="rounded px-3 py-2 text-left text-xs hover:bg-[var(--cfip-terminal-surface-active)]">{localeNames[x]}</button>)}</div>}
      {panel==="settings"&&<div data-terminal-panel className="absolute right-2 top-11 z-50 w-72 rounded-lg border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)] p-3 shadow-2xl">
        {([
          ["showGrid","grid"],["showVolume","volume"],["showSessions","sessions"],["showBidAsk","bidAsk"],["magnet","magnet"]
        ] as const).map(([key,label])=><label key={key} className="flex items-center justify-between border-b border-[var(--cfip-terminal-border-subtle)] px-2 py-2.5 text-xs last:border-0"><span>{t(locale,label)}</span><input type="checkbox" checked={prefs[key]} onChange={e=>setPrefs({...prefs,[key]:e.target.checked})}/></label>)}
      </div>}
    </header>
    <div className="flex min-h-0 flex-1">
      {rail&&<nav className="cfip-terminal-rail flex w-12 shrink-0 flex-col items-center gap-1 border-r border-[var(--cfip-terminal-border)] bg-[var(--cfip-terminal-surface)] py-2">{DRAWING_TOOLS.map(x=><button key={x} onClick={()=>setTool(x)} title={tt(x)} aria-label={tt(x)} className={`h-9 w-9 rounded text-xs ${tool===x?"bg-[var(--cfip-terminal-accent-strong)] text-white":"text-[var(--cfip-terminal-text-muted)] hover:bg-[var(--cfip-terminal-surface-active)]"}`}>{TOOL_GLYPHS[x]}</button>)}</nav>}
      <section className="relative min-w-0 flex-1" onClick={placeDrawing}>
<div className="absolute left-3 top-8 z-20 flex gap-2 text-[10px] text-[var(--cfip-terminal-text-faint)]">
          ...
          <strong className="text-white">{symbol}</strong><span className="text-[var(--cfip-terminal-text-muted)]">{tf}</span>
          {last&&<><span>O {last.open.toFixed(meta?.digits??5)}</span><span>H {last.high.toFixed(meta?.digits??5)}</span><span>L {last.low.toFixed(meta?.digits??5)}</span><span>C {last.close.toFixed(meta?.digits??5)}</span><span className={pct>=0?"text-[var(--cfip-terminal-success)]":"text-[var(--cfip-terminal-danger)]"}>{pct>=0?"+":""}{pct.toFixed(2)}%</span></>}
          <span className={live?"text-[var(--cfip-terminal-success)]":"text-[var(--cfip-terminal-warning)]"}>● {live?t(locale,"live"):error?t(locale,"dataOffline"):t(locale,"loading")}</span>
        </div>
        {zones.length > 0 && <div className="pointer-events-none absolute left-3 bottom-10 z-10 rounded border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)]/85 px-2 py-1 text-[10px] text-[var(--cfip-terminal-chart-text)]">{zones.length} FVG zones</div>}
        {levels.length > 0 && <div className="pointer-events-none absolute right-3 bottom-10 z-10 rounded border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)]/85 px-2 py-1 text-[10px] text-[var(--cfip-terminal-chart-text)]">{levels.length} S/R levels</div>}
        {session && prefs.showSessions && <div className="pointer-events-none absolute left-3 bottom-20 z-10 rounded border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)]/85 px-2 py-1 text-[10px] text-[var(--cfip-terminal-chart-text)]">Session {session.low.toFixed(meta?.digits??5)} — {session.high.toFixed(meta?.digits??5)}</div>}
        {prefs.showBidAsk && last && (last.bid || last.ask) && <div className="pointer-events-none absolute right-3 top-8 z-20 rounded border border-[var(--cfip-terminal-border-strong)] bg-[var(--cfip-terminal-surface)]/90 px-2 py-1 text-[10px] tabular-nums text-[var(--cfip-terminal-text)]">{last.bid && <span className="mr-3 text-[var(--cfip-terminal-info)]">B {Number(last.bid).toFixed(meta?.digits??5)}</span>}{last.ask && <span className="text-[var(--cfip-terminal-warning)]">A {Number(last.ask).toFixed(meta?.digits??5)}</span>}</div>}
        <svg key={overlayTick} aria-label="Chart drawings" className="pointer-events-none absolute inset-0 z-10 h-full w-full overflow-visible">
          {zones.map((z,i)=>{
            ...
            return <rect key={'fvg-'+i} x={Math.min(xa,xb)} y={Math.min(ya,yb)} width={Math.max(1,Math.abs(xb-xa))} height={Math.max(1,Math.abs(yb-ya))} fill={z.bullish?"var(--cfip-terminal-success-fill)":"var(--cfip-terminal-danger-fill)"} stroke={z.bullish?"var(--cfip-terminal-success)":"var(--cfip-terminal-danger)"} strokeWidth='1' strokeDasharray='4 3'/>;
          })}
          {drawings.filter(d=>d.visible!==false).map(d=>{
            ...
            if(d.tool==="rectangle") return <g key={d.id}>{hit(x1,y1,x2,y2)}<rect x={Math.min(x1,x2)} y={Math.min(y1,y2)} width={Math.abs(x2-x1)} height={Math.abs(y2-y1)} fill="var(--cfip-terminal-position-fill)" stroke={selectedDrawingId===d.id?TERMINAL_THEME.highlight:TERMINAL_THEME.volumeUp} strokeWidth={selectedDrawingId===d.id?2:1}/></g>;
        ...
        {!candles.length&&<div className="pointer-events-none absolute inset-0 flex items-center justify-center"><div className="rounded-lg border border-[var(--cfip-terminal-border-subtle)] bg-[var(--cfip-terminal-surface)]/95 px-8 py-6 text-center shadow-xl"><div className="text-lg font-semibold">{t(locale,"noData")}</div><div className="mt-2 max-w-lg text-xs leading-5 text-[var(--cfip-terminal-text-muted)]">CFIP renders normalized market observations only. No synthetic candles are generated.</div></div></div>}
      </section>
    ...
    <footer className="cfip-terminal-footer flex h-7 shrink-0 items-center justify-between border-t border-[var(--cfip-terminal-border)] bg-[var(--cfip-terminal-surface)] px-3 text-[10px] text-[var(--cfip-terminal-text-faint)]">
