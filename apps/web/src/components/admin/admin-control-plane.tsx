...
export function AdminControlPlane() {
  ...
  const identityName = config?.intelligence_identity.name ?? runtime?.app.name ?? "Market Intelligence";
  return <main className="min-h-dvh bg-[var(--cfip-terminal-bg)] text-[var(--cfip-terminal-text)]">
    <header className="sticky top-0 z-20 flex h-12 items-center border-b border-[var(--cfip-terminal-border)] bg-[var(--cfip-terminal-surface)]/95 px-4 backdrop-blur">
      <a href="/" className="font-semibold tracking-[0.14em] text-white">{identityName}</a><span className="mx-3 text-[var(--cfip-terminal-border-strong)]">/</span><span className="text-xs text-[var(--cfip-terminal-text-muted)]">CONTROL PLANE</span>
    ...
    <div className="mx-auto flex max-w-[1500px] min-h-[calc(100dvh-3rem)]">
      ...
      <section className="min-w-0 flex-1 p-5 md:p-8">
        ...
        </Card><div className="mb-6 flex items-start justify-between gap-4"><div><div className="text-[10px] uppercase tracking-[0.2em] text-[var(--cfip-terminal-text-faint)]">{identityName} control plane</div><h1 className="mt-1 text-2xl font-semibold text-white">{active.id === "intelligence" ? identityName : active.title}</h1><p className="mt-1 max-w-3xl text-sm text-[var(--cfip-terminal-text-muted)]">{active.description}</p></div><span className="rounded-full border border-[var(--cfip-terminal-border-strong)] px-2.5 py-1 text-[10px] uppercase tracking-wider text-[var(--cfip-terminal-text-muted)]">{active.maturity}</span></div>
