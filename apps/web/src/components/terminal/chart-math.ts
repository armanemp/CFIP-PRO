...
export function vwap(c:Candle[]){
  let pv=0, v=0, day="";
  return c.map(x=>{
    const currentDay=new Date(Number(x.time)*1000).toISOString().slice(0,10);
    if(currentDay!==day){day=currentDay;pv=0;v=0;}
    const n=x.volume>0?x.volume:1;
    pv+=((x.high+x.low+x.close)/3)*n;
    v+=n;
    return {time:x.time,value:pv/v};
  });
}
export function bollinger(c:Candle[],p=20,multiplier=2){return c.flatMap((x,i)=>{if(i+1<p)return [];const w=c.slice(i+1-p,i+1).map(q=>q.close),m=w.reduce((s,q)=>s+q,0)/p,sd=Math.sqrt(w.reduce((s,q)=>s+(q-m)**2,0)/p)*multiplier;return[{time:x.time,upper:m+sd,mid:m,lower:m-sd}];});}
...
export function atr(c:Candle[],p=14){
  ...
  let value=tr.slice(1,p+1).reduce((s,q)=>s+q,0)/p;
  const out=[{time:c[p].time,value}];
