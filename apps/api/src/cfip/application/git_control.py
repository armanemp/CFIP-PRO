...
from cfip.domain.git_control import GitChangeProposal, GitScope, direct_main_commit_allowed, proposal_paths_allowed, is_protected_path
...
class GovernedGitService:
    ...
    def commit(self, proposal: GitChangeProposal) -> dict[str, object]:
        ...
        if not proposal_paths_allowed(proposal, scope):
            raise PermissionError("git_path_not_allowed")
        protected = any(is_protected_path(path, scope) for path in proposal.paths)
        ...
        if protected and decision.risk in {"low", "medium"}:
