"""Canonical runtime-editable identity for the entire platform."""

from pydantic import BaseModel, ConfigDict, Field, field_validator

DEFAULT_INTELLIGENCE_NAME = "MIOSAI"


class IntelligenceIdentity(BaseModel):
    model_config = ConfigDict(extra="forbid")

    name: str = Field(default=DEFAULT_INTELLIGENCE_NAME, min_length=2, max_length=48)
    short_name: str = Field(default="MIOSAI", min_length=2, max_length=12)
    domain: str = Field(default="finance.market-intelligence", min_length=3, max_length=64)
    description: str = Field(
        default=(
            "Advanced evidence-grounded financial market intelligence with learning, "
            "self-healing and governed self-development."
        ),
        min_length=8,
        max_length=240,
    )

    @field_validator("name", mode="before")
    @classmethod
    def normalize_name(cls, value: object) -> object:
        if isinstance(value, str):
            return " ".join(value.strip().split())
        return value

    @field_validator("short_name", mode="before")
    @classmethod
    def normalize_short_name(cls, value: object) -> object:
        if isinstance(value, str):
            return " ".join(value.strip().split())
        return value


DEFAULT_INTELLIGENCE_IDENTITY = IntelligenceIdentity()
