"""Persist governed signal lifecycle and outcome analytics."""
...
revision: str = "0003_outcome_analytics"
...
def upgrade() -> None:
    ...
    op.create_index("ix_signal_lifecycles_symbol_timeframe_decision", "signal_lifecycles", ["symbol", "timeframe", "decision_time"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["signal_id"], ["signal_lifecycles.signal_id"], ondelete="RESTRICT"),
    )
    op.create_index("ix_signal_outcome_events_signal_time", "signal_outcome_events", ["signal_id", "event_time"])
    ...
    op.create_index("ix_signal_outcomes_label_evaluation_time", "signal_outcomes", ["label", "evaluation_time"])
    ...
    op.create_table(
        ...
        sa.Column("expected_calibration_error", sa.Numeric(20, 10), nullable=True),
        sa.Column("bins", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("ix_calibration_reports_as_of", "calibration_reports", ["as_of"])
...
def downgrade() -> None:
    ...
    op.drop_index("ix_signal_outcomes_label_evaluation_time", table_name="signal_outcomes")
    op.drop_table("signal_outcomes")
    op.drop_index("ix_signal_outcome_events_signal_time", table_name="signal_outcome_events")
    op.drop_table("signal_outcome_events")
    op.drop_index("ix_signal_lifecycles_state", table_name="signal_lifecycles")
    op.drop_index("ix_signal_lifecycles_symbol_timeframe_decision", table_name="signal_lifecycles")
